...
def test_pass_lead_endpoint_final():
    """Test the pass lead endpoint with proper verification"""
    ...
    print("🔍 Testing Pass Lead Endpoint Flow (Final)...")
    ...
    # Step 6: Verify lead status changed via admin endpoint
    print("Step 6: Verifying lead status via admin...")
    response = session.post(f"{base_url}/api/auth/login", json=admin_creds)
    if response.status_code != 200:
        print(f"❌ Failed to login as admin for verification: {response.status_code}")
        return False
    
    response = session.get(f"{base_url}/api/admin/leads")
    if response.status_code != 200:
        print(f"❌ Failed to get all leads: {response.status_code}")
        return False
    ...
    if passed_lead and passed_lead.get('status') == 'PASSED':
        ...
        # Check if passed_at timestamp was set
        if 'passed_at' in passed_lead:
            print(f"✅ Lead has passed_at timestamp: {passed_lead.get('passed_at')}")
        else:
            print(f"⚠️ Lead missing passed_at timestamp")
...
if __name__ == "__main__":
    success = test_pass_lead_endpoint_final()
    if success:
